...
void Simulation::applyForces(){
	...
	for(int i=0; i<N; i++){
		dens[i] = 0.0f;
		for(int j=0; j<N; j++){
			dens[i] += kernel(x[i] - x[j], kernelRadius);
		}
		dens[i] *= pm;
	}
	...
	for(int i=0; i<N; i++){
		...
		for(int j=0; j<N; j++){
			//Presure force
			a[i] += - ( dt * kappa * (dens[j] - dens_0) / dens[j]) * dkernel(x[i] - x[j], kernelRadius);

			//Viscosity force
			a[i] -= dt * mu * dx[j] / dens[j] * ddkernel(x[i] - x[j], kernelRadius);
...
bool Simulation::collideAndMove(int index, glm::vec3 &particleStep){
	...
	dx_2[i] = dx_2[i] - (1.0f + c_R)*glm::dot(n,dx_2[i])*n;
